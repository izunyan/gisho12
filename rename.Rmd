...
* dfデータで"_"を含む変数の名前から，"_"をすべて取り除こう（ヒント^[str_replace( )`も`str_remove( )`も適用は最初の1つだけなので，複数ある場合は`str_replace_all( )`または`str_remove_alla( )`を使う]）
...
* ここでは変数year以外すべてなので，yearに`!`をつけることで変数を指定できる
