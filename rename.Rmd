...
* 語の置き換えに`stringr::str_replace()`を適用してみたい
  + 第一引数に対して，その次の文字列をその後の文字列に置換する（ここでは"bill" → "くちばし"）
* 第一引数の「`x`」の部分に，その後選ぶ列が入っていく（ここでは`starts_with("bill")`）
  + この部分の理解は今すぐできなくてもよいが，学習を進めるためにキーワードだけ示しておくと，**無名関数**(anonymous function)という処理をしている(\@ref(p-function-make-anonymous)参照)
...
df |> 
  rename_with(\(x) str_replace(x, "_mm", "_ミリ"),
              ends_with("_mm"))
```
