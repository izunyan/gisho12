...
* 好みまたは目に優しい画面にするため，RStudioの色合いを下記から変更できる
  + メニューの`Tools > Global Options > Appearance > Editor theme`から選び，OKを押す
...
* 下図の〇で囲んである左上の緑`＋`がついているアイコンから，または`File > New file > R Script`でR Scriptを開く 

```{r script, echo = FALSE, fig.cap = "Rスクリプトを開く", fig.align = "center", out.width='70%'}
knitr::include_graphics("images/newfile.png")
```
* R Scriptを開くとデフォルトでは左上にペインが開き，ここにコードを記述できる
...
* `Tools > Global Options > Code > Saving > Default text encoding`からUTF-8選び，OKを押す
...
* 本書では最初の3行の表示のみに絞っているが，任意の行数を表示させたい場合は，`print( )`関数を使う
...
* 関数の使い方，引数の入れ方は大抵忘れるので，ヘルプのexampleを見て思い出す場合が多い
  + コンソールに`?関数名`の用に入れて実行する（例：?rename）
  + 右下ペインのHelpタブで検索する
