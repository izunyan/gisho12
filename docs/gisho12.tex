...
\title{がんばらないデータ加工　ーRによる繰り返し作業入門ー　前編}
\author{やわらかクジラ}
\date{}
...
\hypersetup{
  pdftitle={がんばらないデータ加工　ーRによる繰り返し作業入門ー　前編},
  pdfauthor={やわらかクジラ},
  hidelinks,
...
本書『\textbf{がんばらないデータ加工　ーRによる繰り返し作業入門ー}』を書こうと思ったのは拙既刊書\href{https://izunyan.github.io/excel_r/}{『Rで読むExcelファイル』}と同じく，「RとRStudioを使いたい！と思う人がもっと増えればいいのに」という願いから
...
本書の内容は、githubレポジトリの \href{https://github.com/izunyan}{} ですべて公開しています。コードやサンプルデータはこちらのレポジトリをダウンロードしてお試しください。pdf版が読みたい方は、以下のページで無料で入手可能です。自力でできる方は、\texttt{Build\ Book}でも作成できます。
...
なお、「前篇」とつけたのは，効率化を目指すコードにさらに踏み入っていく場合，リストの理解や\texttt{purrr}パッケージの使い方などを丁寧に説明する必要があるため，
...
{\centering \includegraphics[width=15.88in]{images/package_install} 
...
{\centering \includegraphics[width=15.67in]{images/dfr} 
...
{\centering \includegraphics[width=13.76in]{images/df_xl} 
...
\tightlist
\item
  名前は \textbf{パイプ} で発音は'' \emph{and then} '' (参照){[}\url{https://adv-r.hadley.nz/functions.html\#function-composition}{]}
\item
  ...
  \tightlist
  \item
    主に使用が想定される場面でざっくりいうと，「このデータフレームに対して\%\textgreater\%の後にある関数を適用する」という機能
  \item
...
\hypertarget{ux88dcux8db3-ux306eux610fux5473}{%
\subsection{【補足】\%\textgreater\% の意味}\label{ux88dcux8db3-ux306eux610fux5473}}
...
\item
  ...
  \item
    身長や質量(mass)の連続量データに加え，色や種族など豊富なカテゴリを持つ変数がある
  \end{itemize}
\end{itemize}
...
\hypertarget{ux8907ux6570ux6761ux4ef6}{%
\section{複数条件}\label{ux8907ux6570ux6761ux4ef6}}
...
\hypertarget{ux52b9ux7387ux5316}{%
\subsection{【効率化】}\label{ux52b9ux7387ux5316}}
...
\hypertarget{ux30adux30fcux30efux30fcux30c9ux306bux3088ux308bux691cux7d22}{%
\section{キーワードによる検索}\label{ux30adux30fcux30efux30fcux30c9ux306bux3088ux308bux691cux7d22}}
...
\item
  ...
  \item
    list内の関数\texttt{()}内に\texttt{.x}が必要（この場合はxがなくてピリオドのみでも動く）。ここに\texttt{across()}の第一引数に指定した変数が入っていくという意味
  \end{itemize}
\end{itemize}
...
\tightlist
\item
  変数の四則演算の式を入れれば合計得点として計算された列をデータフレーム追加できる
\item
  別解で合計した例を，変数の逆転項目を反映させた後に \ref{mu-total-ef} で解説する
...
\tightlist
\item
  変数ageのヒストグラムを描き，中央値を確認する
\item
  単純に確認したいだけなら，従来のRでの書き方が楽
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# ヒストグラム描画}
\FunctionTok{hist}\NormalTok{(df\_bfi}\SpecialCharTok{$}\NormalTok{age)}
\end{Highlighting}
\end{Shaded}

\includegraphics{gisho12_files/figure-latex/unnamed-chunk-76-1.pdf}
...
\tightlist
\item
  \texttt{dplyr}パッケージの\texttt{if\_else(\ )}関数で条件式（TRUEまたはFALSEを返すもの）によって値を2区分する
\item
...
\tightlist
\item
  \texttt{age\ \textgreater{}=\ 27}が1，27未満が0にコーディングされているかfilterで限定して確認
\end{itemize}
