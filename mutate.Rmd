...
* df_bfiデータの変数A1とA2だけを選んで，A2からA1を引いた新しい変数"a1_2"を作成しよう
...
* df_bfiデータの変数A1, A2, A3, A4, A5だけを選んで，すべて因子型に変換しよう
...
* `list()`に入れるときはこれまでと異なる書き方が必要になる
  ...
  + list内の関数`()`内に`.x`が必要。この`.x`に`across()`の第一引数に指定した変数が入っていくという意味
